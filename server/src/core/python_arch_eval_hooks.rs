...
static arch_eval_function_hooks: LazyLock<Vec<PythonArchEvalFunctionHook>> = LazyLock::new(|| {vec![
    ...
    // Environment attributes are annotated BaseModel and refined to their model by a symbol hook.
    ...
    // companies is a recordset of res.company, with no multiplicity distinction here
...
impl PythonArchEvalHooks {
    ...
    /// Make the file of `scope` depend on `model`, unless it is an orm file itself.
    ...
    /// Resolve a fixed model name to its main class instance at evaluation time.
